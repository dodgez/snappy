...
pub fn get_latest_commit() -> Result<String, io::Error> {
    ...
    let branch = read_to_string(repo.head_file)?;
    let branch_file = repo.branches_dir.join(&branch);
    if !branch_file.exists() {
        let commit_file = repo.snaps_dir.join(get_hash_path(&branch));
...
pub fn update_branch(commit: &str) -> Result<(), io::Error> {
    ...
    if !branch_file.exists() {
        ...
        write(repo.head_file, commit.as_bytes())?;
    }
...
pub fn branch(branch: &str) -> Result<(), io::Error> {
    ...
    write(branch_file, commit.as_bytes())?;
    write(repo.head_file, branch.as_bytes())?;
